...
#sh upgrade_pm_hive.sh 172.18.0.1 5432 i2b2 demouser i2b2 i2b2demodata i2b2metadata i2b2pm i2b2hive i2b2workdata i2b2-core-server 8080
...
export pm_sql="set search_path to i2b2pm ; delete from PM_CELL_DATA;INSERT INTO PM_CELL_DATA (CELL_ID, PROJECT_PATH, NAME, METHOD_CD, URL, CAN_OVERRIDE, STATUS_CD) VALUES('CRC', '/', 'Data Repository', 'REST', 'http://i2b2-core-server:8080/i2b2/services/QueryToolService/', 1, 'A'),('FRC', '/', 'File Repository ', 'SOAP', 'http://i2b2-core-server:8080/i2b2/services/FRService/', 1, 'A'),('ONT', '/', 'Ontology Cell', 'REST', 'http://i2b2-core-server:8080/i2b2/services/OntologyService/', 1, 'A'),('WORK', '/', 'Workplace Cell', 'REST', 'http://i2b2-core-server:8080/i2b2/services/WorkplaceService/', 1, 'A'),('IM', '/', 'IM Cell', 'REST', 'http://i2b2-core-server:8080/i2b2/services/IMService/', 1, 'A');"
...
docker exec -i -e PGPASSWORD=$TARGET_PASSWORD i2b2-data-pgsql psql -h $TARGET_SERVER -U $TARGET_USER -d $TARGET_DB_NAME -f /tmp/pm_sql.sql

docker exec -i -e PGPASSWORD=$TARGET_PASSWORD i2b2-data-pgsql psql -h $TARGET_SERVER -U $TARGET_USER -d $TARGET_DB_NAME -f /tmp/hive_sql.sql
